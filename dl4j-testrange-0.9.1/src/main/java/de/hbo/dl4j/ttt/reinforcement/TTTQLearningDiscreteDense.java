...
public class TTTQLearningDiscreteDense<O extends Encodable> implements ILearning<O, Integer, DiscreteSpace>, NeuralNetFetchable<IDQN<?>> {
    ...
    public TTTQLearningDiscreteDense(
        ...
        currentDQN = new BummBummDQN(netConf).buildDQN(mdp.getObservationSpace().getShape(), mdp.getActionSpace().getSize());        //new DQNFactoryStdDense(netConf).buildDQN(mdp.getObservationSpace().getShape(), mdp.getActionSpace().getSize());
        targetDQN = currentDQN.clone();
        policy = new DQNPolicy<>(currentDQN);
        egPolicy = new EpsGreedy<>(policy, mdp, conf.getUpdateStart(), conf.getEpsilonNbStep(), random, conf.getMinEpsilon(), this);
	...
	public void train() {
        ...
        try {
            ...
            while (stepCounter < getConfiguration().getMaxStep()) {
                ...
                if (stepCounter - lastMonitor >= Constants.MONITOR_FREQ && historyProcessor != null && dataManager.isSaveData()) {
                    lastMonitor = stepCounter;
                    int[] shape = mdp.getObservationSpace().getShape();
                    historyProcessor.startMonitor(dataManager.getVideoDir() + "/video-" + epochCounter + "-" + stepCounter + ".mp4", shape);
                }
                        ...
                        if (stepCounter % skipFrame == 0 || stepReply.isDone()) {
                            ...
                            if (stepCounter > updateStart) {
                                ...
                                {//setTarget() 
                                    ...
                                    INDArray dqnOutputNext = currentDQN.output(nextObs);
                                    if (configuration.isDoubleDQN()) {
                                        targetDqnOutputNext = targetDQN.output(nextObs);
                                        getMaxAction = Nd4j.argMax(dqnOutputNext, 1);
                                    } else tempQ = Nd4j.max(dqnOutputNext, 1);
                                    ...
                                    for (int i = 0; i < transitions.size(); i++) {
                                        ...
                                        if (!areTerminal[i]) {
                                            ...
                                            if (configuration.isDoubleDQN()) q += targetDqnOutputNext.getDouble(i, getMaxAction.getInt(i));
                                            else q += tempQ.getDouble(i);
                                        ...
                                        double previousV = dqnOutputAr.getDouble(i, actions[i]);
                                        double lowB = previousV - configuration.getErrorClamp();
                                        double highB = previousV + configuration.getErrorClamp();
    ...
    public static int[] makeShape(int batch, int[] shape, int length) {
        ...
        nshape[0] = batch;
        nshape[1] = 1;
        for (int i = 0; i < shape.length; i++) nshape[1] *= shape[i];
        nshape[2] = length;
        return nshape;
    }
	*/
    //protected void setHistoryProcessor(HistoryProcessor.Configuration conf) { historyProcessor = new HistoryProcessor(conf);}
    //public static Integer getMaxAction(INDArray vector) {return Nd4j.argMax(vector, Integer.MAX_VALUE).getInt(0);}

    @Override public IDQN<?> getNeuralNet() {return currentDQN;}
    ...
    @Value
    private static class BummBummDQN implements DQNFactory {
        ...
        public DQN buildDQN(int[] numInputs, int numOutputs) {
            ...
            NeuralNetConfiguration.ListBuilder confB = new NeuralNetConfiguration.Builder().seed(Constants.NEURAL_NET_SEED)
                            ...
                            .weightInit(WeightInit.ZERO) //.weightInit(WeightInit.XAVIER)
