package de.hbo.dl4j.classifier.util;
...
public class RTPlotter {
	...
	private RTPlotter(INDArray backgroundIn, INDArray backgroundOut, int nDivisions, XYDataset xyDataset) {
        ...
        f.getContentPane().add(panel = new ChartPanel(
    		createChart(
				createBackgroundData(backgroundIn, backgroundOut), 
				backgroundIn.min(0).data().asDouble(), 
				backgroundIn.max(0).data().asDouble(), 
				nDivisions, 
				xyDataset
			)
		));
	...
	public void refresh(INDArray backgroundIn, INDArray backgroundOut, int nDivisions) {
     ...
     */
    public static RTPlotter plotTrainingData(INDArray features, INDArray labels, INDArray backgroundIn, INDArray backgroundOut, int nDivisions) {
    	return new RTPlotter(backgroundIn, backgroundOut, nDivisions, createDataSetTrain(features, labels));
    }
     ...
     * @param nDivisions Number of points (per axis, for the backgroundIn/backgroundOut arrays)
     */
    public static RTPlotter plotTestData(INDArray features, INDArray labels, INDArray predicted, INDArray backgroundIn, INDArray backgroundOut, int nDivisions) {
    	return new RTPlotter(backgroundIn, backgroundOut, nDivisions, createDataSetTest(features, labels, predicted));
    }
    ...
    private static XYZDataset createBackgroundData(INDArray backgroundIn, INDArray backgroundOut) {
        ...
        for(int i=0; i<nRows; i++) {
            ...
            zValues[i] = Math.round(backgroundOut.getDouble(i)*10)/10d; //backgroundOut.getDouble(i);
            
            //if (Double.isNaN(min) || backgroundOut.getDouble(i) < min) min = backgroundOut.getDouble(i);
            //if (Double.isNaN(max) || backgroundOut.getDouble(i) > max) max = backgroundOut.getDouble(i);
        }
